...
/**
 * A data access object (DAO) providing persistence and search support for Area entities. Transaction control of the save(), update() and delete() operations can directly support
...
public class AreaDAO {
	...
	public Area findById(java.lang.Integer id) {
		...
		try {
			Area instance = (Area) getCurrentSession().get("com.xxbs.v1.db.Area", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Area> findByExample(Area instance) {
		...
		try {
			List<Area> results = (List<Area>) getCurrentSession().createCriteria("com.xxbs.v1.db.Area").add(create(instance)).list();
	...
	public List<Area> findByLevel(Object level) {
