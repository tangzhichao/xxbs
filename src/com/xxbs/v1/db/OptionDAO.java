...
/**
 * A data access object (DAO) providing persistence and search support for Option entities. Transaction control of the save(), update() and delete() operations can directly support
...
public class OptionDAO {
	...
	public Option findById(java.lang.Integer id) {
		...
		try {
			Option instance = (Option) getCurrentSession().get("com.xxbs.v1.db.Option", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Option> findByExample(Option instance) {
		...
		try {
			List<Option> results = (List<Option>) getCurrentSession().createCriteria("com.xxbs.v1.db.Option").add(create(instance)).list();
	...
	public List<Option> findByQuestionId(Object questionId) {
		return findByProperty(QUESTION_ID, questionId);
	}

	public List<Option> findBySortOrder(Object sortOrder) {
		return findByProperty(SORT_ORDER, sortOrder);
	}

	public List<Option> findByContent(Object content) {
	...
	public Option merge(Option detachedInstance) {
		...
		try {
			Option result = (Option) getCurrentSession().merge(detachedInstance);
