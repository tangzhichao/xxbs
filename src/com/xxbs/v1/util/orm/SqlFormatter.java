...
public class SqlFormatter{
	 ...
	 * 格式化:key格式入参的sql
	 */
	public static String format(String sql,Map mp) {
		sql=sql.replaceAll("&lt;", "<");
		sql=sql.replaceAll("&gt;", ">");
		...
		while(iterator.hasNext()){
			...
			newSql=newSql.replaceAll(":"+key,"'"+Utils.toString(mp.get(key))+"'");
		}
		return new FormatProcess(newSql).perform();
	}
	/**
	 * 格式化?格式入参的sql
	 */
	public static String format(String sql,Object[] mp) {
		sql=sql.replaceAll("&lt;", "<");
		sql=sql.replaceAll("&gt;", ">");
		...
		int i=0;
		while(indexOf!=-1){
			sb.replace(indexOf, indexOf+1, "'"+Utils.toString(mp[i])+"'");
			indexOf=sb.indexOf("?", indexOf+1);
			i++;
		}
		return new FormatProcess(sb.toString()).perform();
	}
	...
	private static class FormatProcess {
		...
		private void updateOrInsertOrDelete() {
			...
			if ("insert".equals(this.lcToken))/* 272 */ this.afterInsert = true;
		}
