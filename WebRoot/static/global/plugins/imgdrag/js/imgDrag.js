...
function WindowMouseMoveAndUp() {
    $(document).mousemove(function (event) {
        ...
        var source_ul = getUlPosition($(document.body).data("_eventx_"), $(document.body).data("_eventy_"));
        var source_li = getPosition($(document.body).data("_eventx_"), $(document.body).data("_eventy_"), source_ul);
        var position_ul = getUlPosition(event.pageX, event.pageY);
        var position_li = getPosition(event.pageX, event.pageY, position_ul);
        if(position_li!=source_li){
            //console.log("这是一个拖拽事件");
            ...
            //console.log("这是一个click事件，不需要处理");
            //这是一个click事件，不需要处理
            $(document.body).data("_isimgdown_", false); //标示是否为鼠标在图片上的按下事件
            $(document.body).data("_eventx_", undefined); //鼠标在图片上按下事件时鼠标的x坐标值
            $(document.body).data("_eventy_", undefined); //鼠标在图片上按下事件时鼠标的y坐标值
            $(document.body).data("_li_", undefined); //鼠标在图片上按下事件时当前的li元素
    
            $(document.body).data("_imgtop_", undefined);
            $(document.body).data("_imgleft_", undefined);
        }
    });
}
//#endregion
