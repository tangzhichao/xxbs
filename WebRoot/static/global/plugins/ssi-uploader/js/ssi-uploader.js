...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $chooseBtn = $('' +
         ...
         '</span>').append(element);
        var $uploadBtn = $('<button id="ssi-uploadBtn" class="ssi-button success ssi-hidden"  type="button">' +
         '<span class="ssi-btnIn">' + this.language.upload + '&nbsp;</span>' +
         '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
        var $clearBtn = $('<button id="ssi-clearBtn" class="ssi-hidden ssi-button info"  type="button">' + this.language.clear +
         '</button>');
        var $abortBtn = $('<button id="ssi-abortBtn" class="ssi-button error ssi-cancelAll ssi-hidden"  type="button"><span class="inBtn">' + this.language.abort + ' </span></button>');
...
//----------------------------UPLOADFILES------------------------------------
            ...
            if (thisS.options.preview) {
                var getTemplate = function (content) {
                    return '<table class="ssi-imgToUploadTable ssi-pending">' +
                     ...
                     '<tr><td><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn"  type="button"><span class="trash10 trash"></span></button></td></tr>' +
                ...
                if (typeof thisS.options.onEachUpload === 'function') {//execute the onEachUpload callback
                    try {
                        thisS.options.onEachUpload({//and return some info
                            ...
                            size: (thisS.toUpload[ii].size / 1024).toFixed(2),
                            type: thisS.toUpload[ii].type,
                            // 个人添加，如果上传成功，调用后续处理函数，并将img接口返回数据传入
                            responseData: data
                        });
                    } catch (err) {
                        console.log('There is an error in onEachUpload callback');
    ...
    var locale = {
        ...
        cn: {
            ...
            clear: '清空',
            drag: '拖放',
            sizeError: '$1 超过最大限制 $2.',// $1=file name ,$2=max file size ie( example.jpg has has exceed the size limit of 2mb)
